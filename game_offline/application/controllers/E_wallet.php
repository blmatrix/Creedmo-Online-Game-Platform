...
class E_wallet extends Application{
    ...
    public function index() {
            ...
            SELECT 
                ...
                CASE withdraw_status 
                        ...
                        WHEN 2 
                            THEN '확인' 
                        WHEN 3 
                            THEN '<span class=complete>완료</span>'
                        WHEN 4 
                            THEN '취소'
                        WHEN 5 
                            THEN '취소'
        ...
        $view_name = $this -> set_view_name('e_wallet');
        $this -> load -> view("front/e_wallet/{$view_name}", $data);
    }
    ...
    public function deposit() {
        if ($this -> input -> method(TRUE) == "GET") {
            ...
            $view_name = $this -> set_view_name('deposit_form');
            $this -> load -> view("front/e_wallet/form/{$view_name}",$data);
        }else {    
            ...
            if (!$user){
                alert("The user is not member of Our Membership");
                exit;
            }
            ...
            $result = $this -> deposit_withdraw_service -> deposit(
            	$user_no, 
            	$transfer_to,
            	$this -> input -> post('sender_name', TRUE),
            	$deposit_amount,
            	$ret_message
            );
    ...
    public function withdraw() {
        if ($this -> input -> method(TRUE) == "GET") {
            ...
            $view_name = $this -> set_view_name("withdraw_form");
            $this -> load -> view("front/e_wallet/form/{$view_name}",$data);
        }else {
            $this -> load -> library('form_validation');
            $this -> load -> model('admin/bank_account');
            ...
            $this -> max_amount = $casino_policy -> max_per_withdrawals;
            ...
            $result = $this -> deposit_withdraw_service -> withdraw(
                $user_no, 
                $bank_account_req,
                $this -> input -> post('transfer_from', TRUE),  
                $this -> get_number($this -> input -> post('withdraw_amount_input', TRUE))
            );
            ...
            if ($result){
                ...
                alert_only($this -> deposit_withdraw_service -> get_message());
                exit;
            }    
        }
    }
    ...
    public function cancel_withdraw(){
        ...
        $this -> load -> library('deposit_withdraw_service');
        $result = $this -> deposit_withdraw_service -> cancel_withdraw($user_no, $withdraw_no);
        
        if ($result){
            $data['result'] ="success";
            $data['message'] ="요청이 잘 처리되었습니다";
            echo json_encode($data);
            exit;
        }else {
            $data['result'] ="failed";
            $data['message'] =$this -> deposit_withdraw_service -> get_message();
            echo json_encode($data);
            exit;
        }
    }
    ...
    public function greater_than_or_equal($amount){
    ...
    public function less_than_or_equal($amount){
    ...
    public function transfer_money_form(){
        ...
        $view_name = $this -> set_view_name("transfer_money_form");
        $this -> load -> view("front/e_wallet/form/{$view_name}",$data);
    }
    ...
    public function transfer_wallet_form(){
        ...
        */
        ...
        $view_name = $this -> set_view_name("transfer_wallet_form");
        $this -> load -> view("front/e_wallet/form/{$view_name}",$data);
    }    
	...
	public function e_wallet_bank(){
        ...
        $this -> template_generator -> set_top_user_data($data);
        $this -> template_generator -> set_footer_data($data);
        $this -> load -> view("front/e_wallet/m_e_wallet_bank",$data);
	}
	public function e_wallet_bank_w(){
	    $this -> load -> model('admin/casino_basic_policy',"CBP");
	    $this -> load -> model('admin/user');
	    $this -> load -> model('admin/wallet');
	    $data = array();
        ...
        $this -> template_generator -> set_top_user_data($data);
        $this -> template_generator -> set_footer_data($data);
        $this -> load -> view("front/e_wallet/m_e_wallet_bank_w",$data);
	}
}
