...
class Coupons extends Application{
    ...
    public function index(){
        ...
        $view_name = $this -> set_view_name('coupons');
        $this -> load -> view("front/coupon/{$view_name}",$data);
    }
    ...
    public function change_coupon_status(){
      ...
      $coupon_no     = $this -> input -> post('coupon', TRUE);
      ...
      if (empty($row)){
          $data['result']  = 'fail';
          $data['message'] = '잘못된 접근 및 쿠폰입니다. 관리자에게 문의해주세요';
      }else {
          $res = $this -> coupon_service -> coupon_deposit($user_no,$coupon_no,$transfer_to, $target_status);
          if ($res){
             $data['result']  = 'success'; 
          }else {
             $data['result']  = 'fail'; 
             $data['message']  = $this -> coupon_service -> get_message(); 
          }
      }
