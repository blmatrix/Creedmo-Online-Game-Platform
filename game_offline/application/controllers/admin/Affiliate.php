...
class affiliate extends CI_Controller {
    ...
    public function agent_commission_record(){
        if ($this -> input -> method(TRUE) == "POST") {
            ...
            if ($data['daterange']){
                if ($data['search_keyword']){
                    $sql .=" AND AC.commission_process_start_time >=".string_to_time( $data['search_date_start'])." AND AC.commission_process_end_time<=".string_to_time( $data['search_date_end']);
                }else {
                     $sql .=" WHERE AC.commission_process_start_time >=".string_to_time( $data['search_date_start'])." AND AC.commission_process_end_time <=".string_to_time( $data['search_date_end']);
                }
            }
