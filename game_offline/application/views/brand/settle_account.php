					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Settlement Account</span></li>
							</ol>
					...
					<!-- start: page -->
						...
						<section class="panel">
							<header class="panel-heading">
								<h2 class="panel-title">Settlement Account</h2>
							</header>
							<div class="panel-body">
								...
								<h4 class="text-weight-bold text-dark">Inquiry period  &nbsp;: <?= $daterange? $daterange : 'Total Settlement'  ?></h4>
								    <table class="table table-striped table-bordered table-condensed">
                                    <thead>
                                    <tr  style="background:#f5f5f5; ">
                                        <th class="text-center">Completed Settlement</th>
                                        <th class="text-center">Requested Settlement</th>
                                        <th class="text-center">Total Settlement</th>
...
<?php foreach($agent_settlements as $agent_settlement):?>
										<tr  class ="<?= $agent_settlement-> settlement_status == WITHDRAW_STATUS_NOT_CHECKED?'warning': ''?>">
											...
											<td><?= $agent_settlement -> bank_name?></td>
											<td><?= $agent_settlement -> bank_account?></td>
											...
											<td class="text-right">
                                                ...
                                                <button type="button" class="btn btn-xs btn-danger"  onclick= "changeAgentSettleState('<?=$agent_settlement-> settlement_status?>','<?=WITHDRAW_STATUS_BEFORE_CONFIRM?>','<?= $agent_settlement-> agent_settlement_no?>');" > <i class="fa fa-bell-slash-o"></i> Check</button>
                                                <?php endif;?>
                                                <?php if ($agent_settlement-> settlement_status  == WITHDRAW_STATUS_BEFORE_CONFIRM):?>
                                                <button type="button" class="btn btn-xs btn-primary" data-toggle="confirmation" onclick = "changeAgentSettleState('<?=$agent_settlement-> settlement_status?>','<?= WITHDRAW_STATUS_CONFIRMED?>','<?= $agent_settlement-> agent_settlement_no?>');" >Confirm</button>
