...
class Affiliate_service {
    ...
    public function generate_agent_commission_record($conditions = NULL){
    ...
    public function generate_weekly_agent_commission_record($conditions = NULL){
        try{
        	$this -> CI -> db -> trans_start();    
            if (empty($conditions))
                throw new Exception("Date must be passed");
            
            $agent_result = $this -> generate_agent_commission_record($conditions);
            if (is_bool($agent_result) || $agent_result == FALSE){
                throw new Exception($this -> message);
            }
            
            if (count($agent_result['agent_list']) < 1) {
                log_message('error', "정산 받을 agent 가 존재하지 않습니다");
				$this -> message = "정산 받을 agent 가 존재하지 않습니다";
                return TRUE;
                exit ;
            } else {
            ...
            $this -> CI -> load -> model('admin/agent_commission_record');
			
            /*
			 * agent commission record에 batch insert 할 데이타 배열 
			 * Wallet 에 Agent settlem balance 배치 업데이트는 직접 batch update query를 조합하여 실행
			 */
            $commission_insert_batch_arr = array();
			
			/*
			 * Wallet 에 Agent settlem balance 배치 업데이트는 직접 batch update query를 조합하여 실행
			 */
			$i = 0; 
			$commission_update_sql = "UPDATE wallet SET agent_settlement_balance = CASE";
			$commission_update_in_sql = " WHERE user_no in (";
            
            foreach($agent_result['agent_list'] as $agent_settle_vo ){
                $commission_insert_batch_arr[] = array(
                    ...
                    'commission_process_start_time'  => strtotime($conditions[DATE_CONDITION][START_TIME]),
                    'commission_process_end_time'    => strtotime($conditions[DATE_CONDITION][END_TIME]),
                    'status'                   =>'Y',
                    'reg_date'                 => time()
                );
				
				$commissin_user_no = $agent_settle_vo -> user_no;
				$commisson_amount  = $agent_settle_vo -> deps1_total_commission + $agent_settle_vo -> deps2_total_commission;
				$commission_update_sql .=" WHEN user_no = {$commissin_user_no} THEN agent_settlement_balance + {$commisson_amount}";
				$commission_update_in_sql .= $commissin_user_no;
				if($i != count($agent_result['agent_list']) -1)
                        $in_sql .=",";
				$i++;
            }
			
            /*
			 *위에서 생성된 주간 agent commission 확정 정보를 agent_commission_table 에 기록 
             */
            $this -> CI -> agent_commission_record -> insert_batch($commission_insert_batch_arr);
			
			/*
			 * 위에서 생성된 주간 agent commission 정보를 바탕으로 커미션 wallet 의 
			 * 커미션 금액을 증액 업데이트 
             */
            // batch update query 완성 
            
            $commission_update_sql .= " ELSE agent_settlement_balance END";
			$commission_update_in_sql .= ")";
			$commission_update_sql .= $commission_update_in_sql;
			log_message('error', "agent settlement commission_batch query");
            log_message('error', $commission_update_sql);
            $this -> db -> query($commission_update_sql );
            
            $this -> message  = "process_agent_commision_record is finished successfully ...........";
